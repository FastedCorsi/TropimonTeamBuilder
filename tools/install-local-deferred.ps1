...
$jars = @(Get-ChildItem -LiteralPath $PSScriptRoot -Filter 'TropimonTeamBuilder-*-LOCAL.jar' -File)
...
& (Join-Path $PSScriptRoot 'InstallManagedLocalMod.ps1') -SourceJar $jars[0].FullName `
    -ExpectedModId 'tropimon_team_saver' -LauncherRoot $LauncherRoot -PollSeconds $PollSeconds
exit $LASTEXITCODE
